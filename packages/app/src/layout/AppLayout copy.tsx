...
import { Nav, NavList, NavItem, Page, PageSidebar } from '@patternfly/react-core';
...
export const AppLayout1: FunctionComponent<IAppLayout> = ({ children }) => {
    ...
    <Nav id="nav-primary-simple" role="navigation" theme="dark" aria-label={'global'}>
      <NavList id="nav-list-simple">
        ...
        <NavItem id={'candidates'}>
          <NavLink to={'/candidate-management'}>{t('candidateManagement')}</NavLink>
        </NavItem>
        <NavItem id={'project'}>
          <NavLink to={'/project-management'}>{t('projectManagement')}</NavLink>
        </NavItem>
        <NavItem id={'projects'}>
          <NavLink to={'/projects-calendar'}>{t('projectsCalendar')}</NavLink>
        </NavItem>
      </NavList>
    </Nav>
  );
