...
import {
  ...
  RowAction,
  useGetResourceRequestByIdLazyQuery,
  useGetResourceRequestsQuery,
  useHandleResourceRequestActionsMutation,
} from '@app/models';
...
export const ResourceRequestsTable = () => {
  ...
  useGetResourceRequestsQuery({
    ...
    onCompleted: (data) => {
      setRows(
        data?.sharedResourceRequest?.map((s, index) => {
          return {
            id: s?.id,
            project: s?.project,
  ...
  const [getResourceRequestById] = useGetResourceRequestByIdLazyQuery({
    ...
    onCompleted: (data) => {
      const modalObj = [
        ...
        { key: 'Manager', value: data.sharedResourceRequestById?.requester?.firstName },
      ...
      options: {
        ...
        customBodyRender: (value) => {
          return (
            <Chip
              label={value}
              color={value == ResourceRequestStatus.Completed ? CHIPTYPE.SUCCESS : CHIPTYPE.WARNING}
            />
          );
        },
      },
